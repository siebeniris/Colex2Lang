...
from src.multirep.utils import inertia_score, linkage_matrix
from src.multirep.langcodes import Baseline_Langs


class ClusterData:
    def __init__(self, X, labels, name, max_clusters=None, method="average",
                 metric="cosine", Z=None):
        self.X = X  # data matrix (1092,64)
        self.Z = Z  # linkage matrix (1091,4)
        self.Y = None  # pairwise distance matrix (595686,)
    ...
    def compute_LinkageMatrix(self):
        ...
        self.Z, self.Y = linkage_matrix(self.X, method=self.method)
    ...
    def print_status(self, best=1):
        print(f"ClusterData_name = {self.name}")
        print(f" #lang {len(self.labels)} X.shape[1]= {self.X.shape[1]}")
        print(f" sil[2,]=[{', '.join([s for s in self.silhouette])}]")
    ...
    def compute_scores_and_clusters(self):
        ...
        for k in range(2, self.max_clusters + 1):
            ...
            # the mean intra-cluster distance and the mean nearest-cluster distance
    ...
    # method for returning silhouette and number of clusters
    ...
    def plot_cluster_analysis(self, name="", lang_names=True, corpus="gold", path="", elbow=True, title=True):
        if name == "":
    ...
    def save_numc_and_dend_plot(self, langs_names, name, path, elbow=True, title=True, color_th=0, ang_rot=90,
                                orient="top", width=13, height=3):
        ...
        if elbow:
            ...
            f, (a1, a2) = plt.subplots(1, 2, figsize=(height * 8.5, height),
        ...
        f.savefig(path + name + ".png", bbox_inches='tight', transparent=True)
        plt.close()

    def plot_dendrogram(self, name="", lang_names=True, corpus="gold", path="data/figs_phytree/", w=5, h=3, ang_rot=90,
                        orient="top", color_th=0, distance_sort='descending', count_sort=False, log_scale=False):
        if name == "":
            name = self.name + "_" + corpus + name
